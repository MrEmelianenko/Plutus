...
DATABASE_PATH = r'database/MAR_23_2019/'
database = [set() for _ in range(4)]


def on_terminate(signum, stack):
    global turn_off_script
    turn_off_script = True


turn_off_script = False
...
def main(i, start_time):
    ...
    while True:
        ...
        if i.value % iteration_size == 0:
            elapsed_time = time.time() - start_time
            print('\r→ Wallets checked: {}k ({:.2f} wallets/sec)'.format(int(i.value / 1000), i.value / elapsed_time),
                  end=' ')
...
if __name__ == '__main__':
    ...
    """
    print('')
    print('Are you ready to get some bitcoins?')
    print('Let\'s go!')
    print('')
    ...
    for c, p in enumerate(os.listdir(DATABASE_PATH)):
        print('\r→ Reading database: ' + str(c + 1) + '/' + str(count), end=' ')
        with open(DATABASE_PATH + p, 'rb') as file:
            if c < half:
                if c < quarter:
                    database[0] = database[0] | pickle.load(file)
                else:
                    database[1] = database[1] | pickle.load(file)
            else:
                if c < half + quarter:
                    database[2] = database[2] | pickle.load(file)
                else:
                    database[3] = database[3] | pickle.load(file)
    ...
    input_processes = input('→ How many threads do you want to use (maximum {}): '.format(max_processes))
    amount_of_processes = int(input_processes)
    processes = [None for cpu in range(amount_of_processes)]

    print('')
    print('→ Running script using {} threads'.format(amount_of_processes))
    print('')

    for cpu in range(amount_of_processes):
        processes[cpu] = multiprocessing.Process(target=main, args=(i, start_time))
        processes[cpu].start()
